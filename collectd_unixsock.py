...
class Collectd():
	...
	print self._cmd('PUTVAL %s' % ' '.join(args))
...
if __name__ == '__main__':
    ...
    c = Collectd('/opt/collectd/var/run/collectd-unixsock', noisy=True)
    #list = c.listval()

    #PUTVAL "testhost/interface/if_octets-test0" interval=10 1179574444:123:456
    options={'interval': 10}
    identifier = "collectd/interface7/if_octets-test0"
    vals = [time.time(),123,456]
    c.putval(identifier, vals, options={'interval': 10})
